if you want to store password in hashing format then you should to ass this code 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // If there were no errors, then insert data into databases
    if (empty($firstname_err) && empty($lastname_err) && empty($email_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        if ($stmt) {
            ...
            // Corrected line - remove the $ before mysqli_stmt_execute
